...
L.GreatCircle = L.Circle.extend({
  initialize: function initialize(position) {
    var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    //default options
    var defaults = {
      clipLat: 65,
      //lat (+/-) used to determine when regular circles might be used. set to false to force render of circle as polygon (no matter what), or true to render it as a normal circle (no matter what)
      clipRad: 2000000,
      //radius (m) at which it will always render a polygon, unless clipLat == true. 
      degStep: 0.5,
      //degrees by which the circle drawing function will step for each polygon -- smaller is more refined. 
      maxCopies: -1,
      //set a maximum number of copies if elements are wrapped -- -1 is no max. 
      wrapElements: true,
      //whether to wrap the elements as multiple copies.
      wrapMarker: true,
      //whether to wrap the bound marker, too
  ...
  bindTo: function bindTo(object) {
    var event = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "drag";
  ...
  _m2km: 1 / 1000,
  //I know this is kind of obvious, but I'm just using it to improve legibility of code logic
  //set the latLng center of the Great Circle	
  setLatLng: function setLatLng(position) {
    this._position = L.latLng(position);
    this.redraw();
  },
  ...
  getBounds: function getBounds() {
    ...
    if (this._polygon) {
      ...
      return L.latLngBounds(this._destination_from_bearing(this._position.lat, this._position.lng, 315, this._options.radius * this._m2km), this._destination_from_bearing(this._position.lat, this._position.lng, 135, this._options.radius * this._m2km));
  ...
  _destination_from_bearing: function _destination_from_bearing(lat, lng, bearing, distance) {
    var round_off = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : undefined;
    ...
    var brng = deg2rad * bearing; //kind of a sad attempt at optimization of these costly trig functions
  ...
  redraw: function redraw() {
    ...
    var lng = this._position.lng; //just for legibility
    ...
    var l4 = this._destination_from_bearing(lat, lng, -180, this._options.radius * this._m2km, 3); //now check for the 4 possible clipping conditions
    ...
    if (this._options.wrapElements === false) this._copies = 0; //now we see if we're rendering a polygon or a circle. there are several conditions that result in the polygon being preferred. it is also possible to override this with the clipLat setting.
    ...
    if ((this._options.radius >= this._options.clipRad || this._clipStatus > 1 || l1[0] >= this._options.clipLat || l3[0] <= this._options.clipLat * -1 || this._options.clipLat === false) && this._options.clipLat !== true) {
      ...
      switch (this._clipStatus) {
        ...
        case 3:
          //bottom clipping -- works best as -180 to 180
          var t_start1 = -180;
          var t_stop1 = 0;
          var t_start2 = 0;
          var t_stop2 = 180;
        ...
        case 4:
          //weird case 4 -- also works bet as -180 to 180
          var t_start1 = -180;
          var t_stop1 = 0;
          var t_start2 = 0;
          var t_stop2 = 180;
      ...
      if (!this._polygon) {
        //create new
        if (this._clipStatus == 1 || this._clipStatus == 4) {
          //multipolygon
        ...
        if (this._addedToMap) {
          //if it's added to the map already, add it to the map. I guess I can imagine cases where you might want to calculate it but not render it to a map, like maybe you want to do something to the LatLng points before rendering them? Anyway, I give you the option.
      ...
      if (this._copies > 0) {
        if (typeof this._circles == "undefined") {
          ...
          //this trims any copies off we don't need
          for (var i in this._circles) {
            if (i > this._copies * 2) {
              this._circles[i].remove();
            }
